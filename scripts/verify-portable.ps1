...
if ([System.IO.Path]::GetExtension($executablePath) -ne '.exe') {
    throw 'the executable must be a portable .exe, not an installer or archive'
}

$stream = [System.IO.File]::OpenRead($executablePath)
try {
    if ($stream.ReadByte() -ne 0x4d -or $stream.ReadByte() -ne 0x5a) {
        throw 'the file is not a Windows PE executable'
    }
    $stream.Position = 0x3c
    $reader = New-Object System.IO.BinaryReader($stream)
    $peOffset = $reader.ReadInt32()
    $stream.Position = $peOffset
    if ($reader.ReadUInt32() -ne 0x00004550) { throw 'the executable has an invalid PE header' }
    if ($reader.ReadUInt16() -ne 0x8664) { throw 'the executable is not Windows x64 (AMD64)' }
}
finally {
    $stream.Dispose()
}
...
try {
    ...
    if ($versionOutput -notmatch [regex]::Escape("Transferly $ExpectedVersion")) {
        throw "the executable reports the wrong product version: $versionOutput"
    }
    if ($versionOutput -notmatch 'Wire protocol [0-9]+\.[0-9]+') {
        throw "the executable does not report its independently versioned wire protocol: $versionOutput"
    }
