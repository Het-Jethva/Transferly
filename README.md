...
Clone and start Transferly in a foreground terminal on both computers:

```powershell
git clone https://github.com/Het-Jethva/Transferly.git
Set-Location Transferly
go run ./cmd/transferly
```

Use `go run ./cmd/transferly --help` for startup options. Transferly is distributed from source so users do not have to bypass Windows protections for an unsigned downloaded executable.
...
Create and verify a reproducible portable Windows x64 executable:

```powershell
./scripts/build-windows.ps1 -OutputDirectory dist
./scripts/verify-portable.ps1 -Executable dist/transferly.exe -ExpectedVersion dev
```

The script builds twice and fails if the executables differ, excludes fault-injection code, and writes `dist/transferly.exe`. The result needs no installer, separate Go runtime, background service, system-tray process, or administrator privileges. It is built locally from the checked-out source and reports its executable and wire-protocol versions through `dist/transferly.exe --version`.

## Use

Start Transferly in a foreground terminal on both computers with `go run ./cmd/transferly`, or run a locally built `transferly.exe`. Windows Firewall may ask whether to permit inbound connections; allow the process on the network profile you intend to use. If policy blocks it, open **Windows Security → Firewall & network protection → Allow an app through firewall**, ask the device administrator to permit the executable on the intended profile, and retry. Transferly never requests elevation or modifies firewall policy.
...
The integration suite builds and launches real Transferly processes over loopback and interacts with their public terminal interface. CI also runs the targeted verification-code, conflict-name, and manifest-limit fuzz smoke tests, the race detector, benchmarks, and reproducible portable-build checks. Complete commands and optional physical checks are in [the validation guide](docs/validation.md).
...
Behavior that cannot be reached through the public terminal interface -- a corrupted digest on the wire, a deliberately slowed stream, a controllable session clock -- lives behind the `transferly_faults` build tag and is compiled out of default builds. `scripts/build-windows.ps1` fails if fault code is present in the portable executable. Run the injectable variant with:
